...
from rho_config import CONFIG_VERSION
...
def write_file(filename, plaintext, key):
    ...
    f.write(salt)
    f.write(iv)
    #print "version: %s salt: %s iv: %s" % (CONFIG_VERSION, salt, iv)
...
def read_file(filename, password):
    ...
    iv = f.read(16)
    #print "Read version: %s salt: %s  iv: %s" % (ver, salt, iv)

    cont = f.read()
    try:
        return_me = decrypt(cont, password, salt, iv)
    except Exception as e:
        print "Exception while decrypting the configuration file: %s" % e
#        raise
